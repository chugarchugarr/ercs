...
`identityRef` identifies evidence used to evaluate the principal. RAMS MUST NOT interpret it as an asset, token identifier, instrument, position, or right identifier. Facts about an asset or instrument that are independent of the principal, including mutable instrument state, remain the responsibility of the asset's compliance logic at execution time.
...
A mandate authorizes the agent to act on a set of actions on a specific asset. Actions are identified by `bytes32` labels. RAMS treats the labels as opaque values.

An integration's **action profile** defines how an attempted action is mapped to a `bytes32` label. A profile MAY use an operation identifier alone, such as a function selector, or a deterministic commitment to the operation and selected call parameters or asset-defined object identifiers. If a profile derives its label from call data or an object identifier, mandate-construction tooling and every enforcement venue claiming that profile MUST derive the same label from the same relevant inputs.

A value of `type(uint256).max` in `maxTransactionValue` or `maxCumulativeValue` signals "no limit."

Caps are denominated in the asset's transfer quantity: the token amount for [ERC-20](./eip-20.md) and [ERC-1155](./eip-1155.md), and a token count (1 per transfer) for [ERC-721](./eip-721.md). RAMS caps the quantity transferred, not the identifier itself. The mandate's `asset` identifies the target contract; an action profile MAY further narrow authority within that asset by committing selected call parameters, including a token, instrument, position, or right identifier, into the `action` label. RAMS does not interpret such identifiers and compares only the exact `bytes32` label.
...
`ACTION_NOT_ENABLED` states only that the exact requested `bytes32` action label is not enabled for the mandate. Because RAMS treats action labels as opaque, the registry MUST NOT infer whether the mismatch is the operation itself, a parameter or object bound into the label, or another profile-defined component. Applications that need that distinction must interpret the label using their action profile.
...
The `action` label is an opaque `bytes32` chosen by the integration's action profile: a function selector, a keccak256 of a string, a deterministic commitment to selected call parameters, or any other 32-byte identifier matching the labels the principal signed. An action profile MUST define a deterministic derivation rule so mandate-construction tooling and every enforcement venue claiming that profile derive the same label from the same relevant attempted action. In the selector-only profile used by the reference implementation, a selector MUST be left-aligned as `bytes32(selector)`.

The reference token uses the selector-only action profile, so each gated function carries its own selector as a compile-time constant:
...
`gatedByMandate` is in the Reference Implementation. A token MAY write its own gate, including one that derives a parameter-scoped label.
...
Venues 2 and 3 exist because an asset already deployed without RAMS awareness cannot be gated. All three read the same mandate. When two enforcement venues claim the same action profile, they MUST derive the same action label from the same relevant attempted action; otherwise the principal's authority would become venue-dependent. The venues do not combine: a gated token evaluates `msg.sender`, so a call forwarded by an executor is evaluated against the executor rather than the agent. The venue is out of scope of the normative interface.
...
A single `IComplianceProvider` interface is used rather than separate identity and compliance interfaces because identity verification is a logical subset of compliance checking. A compliance provider that declares a principal eligible has already verified that the underlying identity is valid and unrevoked. `identityRef` therefore remains evidence about the principal; it is not overloaded with asset or instrument scope. Static object specificity belongs in the action label when an integration needs it, while mutable instrument state remains with the asset's compliance logic at execution time.

Mandate scope is encoded fully on-chain and bound by the principal's [EIP-712](./eip-712.md) signature, eliminating drift between on-chain state and any off-chain document. Actions are signed as a `bytes32[]` array and written into a mapping at grant time for O(1) enforcement. Keeping action labels opaque lets an integration choose the granularity it needs without expanding the RAMS ABI: one profile can authorize an operation generally while another can bind that operation to selected parameters or an asset-defined object. The `metadata` field is non-normative and does not participate in enforcement.
...
`canExecute` returns a `MandateReason` rather than a bare boolean so an integrator can distinguish the boundary that rejected the request without assigning semantics the registry does not know. `NOT_YET_VALID` means wait; `WRONG_ASSET` and `OVER_TX_CAP` can succeed on a retry with different parameters; `NONEXISTENT`, `EXPIRED`, `REVOKED`, and `OVER_CUMULATIVE_CAP` require the principal to grant, extend, or replace a mandate, and a spent cumulative budget is not restored by `extendMandate`; `AGENT_FROZEN` and `PRINCIPAL_FROZEN` require an enforcer; and `OTHER` carries no remediation that an integrator can infer. `ACTION_NOT_ENABLED` means only that the exact action label is absent. Depending on the action profile, the caller may need to choose a different operation or bound object, or the principal may need to widen the mandate; RAMS cannot distinguish those cases from the opaque label alone.
...
Enforcement is venue-agnostic: RAMS standardizes the mandate, not how it is enforced. Any contract in the agent's path applies it by calling `canExecute`; venues implementing the same action profile therefore preserve the same authority by deriving the same action label. The account-side venues exist because an asset already deployed cannot be changed to gate itself.
...
The reference executor implements the selector-only action profile. It maintains a mapping from each supported action selector to the position of its amount argument; on `execute`, it reads the gated amount from that position in the forwarded calldata, so the gated value is always the value that executes. Actions with no value argument gate at amount 0. The reference emits an event whenever this registry changes, so the amount-position configuration is auditable off-chain. A parameter-scoped profile can supply a different executor or gate that derives its action label according to that profile without changing `IAgentMandate`.
...
This base contract demonstrates the selector-only action profile. Solidity modifiers cannot be overloaded, so an integration using keccak256 labels or parameter-scoped labels writes its own gate or helper that derives the same action label its mandate-construction tooling signs.
...
An action profile's derivation rule is part of the authority boundary. If mandate-construction tooling and an enforcement venue derive different labels from the same attempted action, the principal can sign one scope while the venue enforces another. Parameter-scoped profiles SHOULD publish test vectors covering every bound parameter and SHOULD test that signing tooling and each enforcement venue derive byte-identical labels.
